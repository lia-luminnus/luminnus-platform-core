...
const Auth = () => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim()) {
      toast({ title: 'Atenção', description: 'Por favor, insira seu e-mail.', variant: 'destructive' });
      return;
    }
    ...
    if (error) {
      ...
      toast({ title: 'Sucesso', description: 'E-mail de recuperação enviado! Verifique sua caixa de entrada.' });
      setIsForgotPassword(false);
    }
  };
  ...
  useEffect(() => {
    ...
    if (isAdminEmail(user.email)) {
      navigate(AUTH_URLS.ADMIN_DASHBOARD, { replace: true });
      return;
    }

    // User with active plan goes to dashboard
    if (hasActivePlan) {
      const dashboardPath = redirectTo ? `/dashboard?redirect_to=${encodeURIComponent(redirectTo)}` : '/dashboard';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0B0B0F] via-[#1A1037] to-[#0B0B0F] flex items-center justify-center p-4">
      <div className="w-full max-w-4xl grid lg:grid-cols-2 gap-0 bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
        {/* Left Column - Form */}
        <div className="p-6 lg:p-8 flex flex-col justify-center">
          <button
            onClick={() => navigate('/')}
            className="flex items-center gap-2 mb-6 text-white/50 hover:text-white transition-colors text-sm w-fit"
          >
          ...
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-white mb-2">
              {isForgotPassword ? 'Recuperar senha' : 'Comece agora'}
            </h1>
            <p className="text-white/60 text-base">
              {isForgotPassword
                ? 'Insira seu e-mail para receber as instruções.'
                : 'Conecte sua conta para conversar com a LIA em segundos.'}
            </p>
          </div>

          {/* Botão Google OAuth - Apenas no Login/Cadastro */}
          {!isForgotPassword && (
            <>
              <div className="space-y-4">
                <Button
                  ...
                  disabled={loading}
                  type="button"
                  className="w-full bg-white hover:bg-white/90 text-gray-800 font-bold shadow-xl h-12 text-base rounded-xl flex items-center justify-center gap-3 transition-all transform hover:scale-[1.01]"
                >
                ...
                <div className="relative flex items-center py-2">
                  ...
                  <span className="flex-shrink mx-4 text-white/30 text-[10px] font-medium uppercase tracking-widest">ou use seu e-mail</span>
                  <div className="flex-grow border-t border-white/10"></div>
                </div>
              </div>
            </>
          ...
          <form onSubmit={isForgotPassword ? handleForgotPassword : handleSubmit} className="space-y-3">
            {!isLogin && !isForgotPassword && (
              <div className="space-y-1">
                <label className="text-xs font-medium text-white/70 ml-1">Nome Completo</label>
                <Input
                  placeholder="Seu nome completo"
                  value={fullName}
                  onChange={(e) => setFullName(e.target.value)}
                  className="bg-white/5 border-white/10 text-white h-11 rounded-xl focus:ring-[#00C2FF]/20"
                  disabled={loading}
                />
              </div>
            )}
                ...
                <div className="flex justify-between items-center px-1">
                  ...
                  {isLogin && (
                    <button
                      ...
                      className="text-[10px] text-[#00C2FF]/70 hover:text-[#00C2FF] transition-colors"
                    >
                      Esqueceu sua senha?
                    </button>
                  )}
            ...
            <Button
              type="submit"
              className="w-full bg-[#00C2FF] hover:bg-[#00C2FF]/90 text-white h-11 rounded-xl font-bold transition-all shadow-lg shadow-[#00C2FF]/10 mt-2"
              disabled={loading}
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> :
                (isForgotPassword ? 'Enviar e-mail de recuperação' : (isLogin ? 'Entrar na LIA' : 'Criar minha conta'))}
            </Button>
          </form>
          ...
          <div className="text-center pt-4">
            <button
              type="button"
              onClick={isForgotPassword ? () => setIsForgotPassword(false) : toggleMode}
              className="text-white/40 hover:text-[#00C2FF] text-xs transition-colors"
            >
              {isForgotPassword
                ? 'Voltar para o login'
        ...
        <div className="hidden lg:block relative">
          ...
          <div className="absolute bottom-6 left-6 right-6">
            <h2 className="text-2xl font-bold text-white mb-2">
              IA com consciência cognitiva
            </h2>
            <p className="text-white/80 text-base">
              Fricção zero. Escalabilidade total.
            </p>
          </div>
        </div>
      </div>
