...
export const EditPlanModal = ({ plan, isOpen, onClose, onSave }: EditPlanModalProps) => {
    ...
    useEffect(() => {
        if (plan) {
            ...
            // 🔧 CORREÇÃO CIRÚRGICA 1: Calcular preço anual imediatamente após reset
    ...
    // Calcular preço anual automaticamente quando preço ou desconto mudar
    ...
    const onSubmit = async (values: PlanFormValues) => {
        ...
        if (!plan.id) {
            console.error('[EditPlanModal] ID do plano não encontrado!', plan);
            toast.error("Erro ao salvar", {
                description: "ID do plano não encontrado. O plano pode não ter sido carregado corretamente.",
                duration: 4000,
            });
            return;
        }
