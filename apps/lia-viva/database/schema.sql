...
-- Index para busca vetorial (similaridade)
...
-- View: Estatísticas de conversas por usuário
...
-- View: Resumo de conversas recentes
...
-- Trigger: Atualizar conversations.updated_at quando message é inserida
CREATE TRIGGER update_conversation_timestamp
AFTER INSERT ON messages
FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();

-- Função: Busca semântica de mensagens
...
-- Políticas para function_calls
CREATE POLICY "Users can view function_calls from own conversations"
  ON function_calls FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM conversations 
      WHERE id = function_calls.conversation_id 
      AND user_id = auth.uid()
    )
  );

CREATE POLICY "Users can insert function_calls in own conversations"
