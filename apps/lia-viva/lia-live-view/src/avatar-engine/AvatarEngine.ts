// =====================================================
// AVATAR ENGINE v3.0 - Lógica Limpa
// =====================================================
// SEM aleatoriedade. SEM timers automáticos.
// Expressão muda APENAS quando:
// 1. EmotionEngine processa uma mensagem
// 2. Usuário clica em botão de teste
// 3. Lip-sync durante áudio
// =====================================================
...
export interface AvatarState {
    expression: ExpressionName  // Emoção atual
    isTalking: boolean          // Se está em lip-sync
...
export interface AvatarFrame {
    ...
    transition: number  // Duração do crossfade em ms
}
...
const CONFIG = {
    expressionTransition: 200,   // ms para trocar expressão (suave)
    talkTransition: 0,           // ms para trocar frame de fala (INSTANTÂNEO)
...
class AvatarEngineClass {
    ...
    // =====================================================
    // FLAG DE PUBLICAÇÃO
    // Avatar fica ESTÁTICO até ser publicado do Avatar Studio
    // =====================================================
    private _isPublished: boolean = false

    constructor() {
        console.log('🎭 AvatarEngine v3.1 inicializado (Aguardando publicação)')
    }

    // Publicar avatar - habilita animações
    publish() {
        this._isPublished = true
        console.log('🎭 Avatar PUBLICADO - Animações habilitadas')
        this.notify()
    }

    // Despublicar - volta a ficar estático
    unpublish() {
        this._isPublished = false
        this._expression = 'neutral'
        this.stopTalking()
        console.log('🎭 Avatar DESPUBLICADO - Voltou a ficar estático')
        this.notify()
    }
    ...
    get frame(): AvatarFrame {
        // Se está falando, usa frame de fala
        // Senão, usa frame da emoção
        if (this._isTalking) {
    ...
    // =====================================================
    // DEFINIR EXPRESSÃO (chamado pelo EmotionEngine)
    // =====================================================

    setExpression(expression: ExpressionName) {
        // Avatar estático até ser publicado
        if (!this._isPublished) return

        // Não muda se estiver falando (lip-sync tem prioridade VISUAL)
        if (this._isTalking) {
            // Salvar para usar depois que parar de falar
            this._expression = expression
            // debug: console.log(`🎭 Expressão agendada (em background): ${expression}`)
            return
        }
    ...
    setEmotionTemperature(temp: number) {
        ...
        console.log(`🌡️ Temperatura: ${this._temperature}`)
        this.notify()
    }
    ...
    startTalking() {
        ...
        console.log('🗣️ Lip-sync iniciado')

        // Ciclo: closed → mid → open → mid → closed ...
    ...
    stopTalking() {
        ...
        console.log(`🗣️ Lip-sync parado. Voltando para: ${this._expression}`)

        // Volta para a expressão atual
        this.notify()
    }
    ...
    onIdle() {
        // Não muda automaticamente - só quando EmotionEngine decidir
    }

    // =====================================================
    // APLICAR EMOÇÃO DO GPT (chamado pelo EmotionEngine)
    // =====================================================

    applyEmotion(emotion: ExpressionName) {
        // Verificar se é permitida pela temperatura
