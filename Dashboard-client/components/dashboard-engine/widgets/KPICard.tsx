 ...
 * KPI Card Widget
 * 
 * Exibe um valor de métrica com comparação ao período anterior
 * Mostra tendência (up/down/stable) e delta percentual
 */
...
function formatValue(value: number | undefined, formatType: string = 'currency'): string {
...
function KPICard({ id, config, data, loading, error, isEditMode }: WidgetProps) {
    ...
    if (loading) {
        return (
            <div className={`h-full w-full rounded-2xl p-4 ${colors.bg} border border-white/10 flex items-center justify-center`}>
                ...
                <div className="flex items-center gap-2 mt-auto pt-2 border-t border-white/5">
                    ...
                    {showPrevious && previousValue !== undefined && (
                        <span className="text-[10px] text-gray-500 font-medium">
                            <span className="opacity-50">vs</span> {formatValue(previousValue, formatType)}
                        </span>
                    )}
                </div>
            )}
