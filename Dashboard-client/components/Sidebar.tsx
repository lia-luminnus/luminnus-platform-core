  ...
  return (
    ...
    >
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.id}
              to={item.path}
              className={`flex items-center ${isSidebarCollapsed ? 'justify-center p-3' : 'justify-start px-4 py-3'} rounded-xl premium-transition group relative ${
                isActive
      ...
      <div className="p-4 flex flex-col gap-4 items-center w-full mt-auto">
         ...
         <button onClick={() => alert('Logout')} className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center' : 'justify-start px-4'} p-3 rounded-xl text-gray-400 hover:bg-red-500/10 hover:text-red-500 premium-transition`}>
