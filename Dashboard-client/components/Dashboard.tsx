...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Auto-open Integrations Hub if onboarding is done but integrations aren't
    // REMOVIDO: O usuário quer acessar o dashboard mesmo sem integrações.
    /*
    if (onboarding_completed && !integrations_completed) {
      toast('Conecte suas ferramentas para ativar a LIA por completo!', {
        icon: '🔌',
        duration: 4000
      });
                ...
                <div className="flex items-center gap-1 bg-gray-100 dark:bg-white/10 p-1.5 rounded-xl shadow-inner">
                  {['daily', 'weekly', 'monthly'].map((id) => (
                    <button
                      ...
                      className={`text-[10px] font-black uppercase px-4 py-2 rounded-lg premium-transition ${timeRange === id
